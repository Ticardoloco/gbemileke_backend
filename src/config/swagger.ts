...
// CDN static assets prevent blank screen rendering failures on Vercel
...
const options: swaggerJSDoc.Options = {
  definition: {
    ...
    // Relative URL ensures Swagger automatically targets the current host (Local OR Vercel)
    servers: [
      {
        url: "/",
  ...
  // Captures both local .ts source files and transpiled .js files on Vercel
  apis: [
...
export function setupSwagger(app: Express): void {
  ...
  // Mount the UI interface with CDN overrides
  app.use(
    "/docs",
    swaggerUi.serve,
